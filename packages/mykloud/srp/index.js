const rfc5054 = {
    N_base10:
        '',
    g_base10: '2',
    k_base16: '5b9e8ef059c6b32ea59fc1d322d37f04aa30bae5aa9003b8321e21ddb04e300',
};
// generate the client session class from the client session factory closure
const SRP6JavascriptClientSession = require('thinbus-srp/client.js')(
    rfc5054.N_base10,
    rfc5054.g_base10,
    rfc5054.k_base16
);
let client 

function InfoCredentials(ServerEphemeral, Salt, username, password) {
    client = new SRP6JavascriptClientSession();
    if (Salt) {
        client.step1(username, password);

        var credentials = client.step2(Salt, ServerEphemeral);

        return { clientEphemeral: credentials.A, clientProof: credentials.M1 };
    }
}

function checkProof(ServerProof){
    try {
        
        client.step3(ServerProof);
    } catch (error) {
            throw new Error('Unexpected server prooftttttt');
    }
}

export  {InfoCredentials,checkProof};
